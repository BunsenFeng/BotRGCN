...
class GATClassifier(pl.LightningModule):
    def __init__(self, in_channels, out_channels, dropout, label_user_count, batch_size):
        ...
        self.shallow = nn.Parameter(torch.randn(229580, self.out_channels))

    def forward(self, x): 
        node_features = self.shallow
...
# model
model = GATClassifier(in_channels = 16, out_channels = 16, dropout = 0.5, label_user_count = 11826, batch_size = 128) #batch_size governs each batch how many nodes
