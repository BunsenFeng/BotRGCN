...
class TwiBot20Dataset(Dataset):
    def __init__(self, node_feature_set, batch_size, label_user_count, name): # name = train/dev
        ...
        self.property_features = torch.load(path + 'user_features1.pt').squeeze(1).float()
        self.property_features = (self.property_features - self.property_features.mean()) / sqrt(self.property_features.var())
    ...
    def __getitem__(self, index):
        return self.node_features, self.edge_index, self.label_list, self.edge_type, self.property_features
...
class GATClassifier(pl.LightningModule):
    def __init__(self, in_channels, out_channels, dropout, label_user_count, batch_size):
        ...
        self.batch_size = batch_size
        self.linear = nn.Linear(in_channels, int(out_channels / 2))
        self.linear2 = nn.Linear(out_channels, out_channels)
        self.linear3 = nn.Linear(out_channels, out_channels)
        self.linearP = nn.Linear(int(out_channels / 2), int(out_channels / 2))
        ...
        torch.nn.init.kaiming_uniform(self.linearP.weight, nonlinearity='leaky_relu')
        ...
        self.shallow = nn.Parameter(torch.randn(229580, int(self.out_channels / 2)))

    def forward(self, x): 
        node_features = x[0].squeeze(0)
        property_features = self.shallow
        edge_index = x[1].squeeze(0)
        
        node_features = self.relu(self.linear(node_features))
        property_features = self.relu(self.linearP(property_features))
        node_features = torch.cat((node_features, property_features), dim = 1)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        label = torch.LongTensor(label[:int(0.9 * self.label_user_count)])
        
        node_features = self.dropout_layer(self.relu(self.linear(node_features)))
        property_features = self.dropout_layer(self.relu(self.linearP(property_features)))
        node_features = torch.cat((node_features, property_features), dim = 1)
        ...
        batch_id = torch.randperm(int(0.9 * self.label_user_count))[:self.batch_size].tolist()
